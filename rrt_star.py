...
class RRTStar(RRT):
    ...
    def planning(self, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            rnd = self.get_random_node() # using function from rrt: def get_random_node(self):
            nearest_ind = self.get_nearest_node_index(self.node_list, rnd)  # using function from rrt: def get_nearest_node_index(node_list, rnd_node)
            # TODO Create a new node with steer
            near_node = self.node_list[nearest_ind]
            new_node = self.steer(near_node, rnd, self.expand_dis) # using steer fucntion from rrt: def steer(self, from_node, to_node, extend_length=float("inf"))
            # TODO Compute the cost of the new node taking into account the nearest node, near_node
            new_node.cost = self.calc_new_cost(near_node, rnd) # using function from rrt_star: def calc_new_cost(self, from_node, to_node):
            ...
            # use function from rrt: def check_collision(node, obstacleList, robot_radius), returns True or False
            if self.check_collision(near_node, self.obstacle_list, self.robot_radius) == True:
                # if there is a collision - don't branch to that path
                near_ind = self.find_near_nodes(new_node) # using function from rrt: def find_near_nodes(self, new_node)
                new_node = self.choose_parent(new_node, near_ind) # using function from rrt: def choose_parent(self, new_node, near_inds)

                # check_collision node can be None. Only rewire the path if the node is not None
                if new_node is not None:
                    self.node_list.append(new_node)
                    self.rewire(new_node, near_ind) # using function: def rewire(self, new_node, near_inds)
    ...
    def choose_parent(self, new_node, near_inds):
        ...
        for i in near_inds:
            ...
            curr_node = self.steer(self.node_list[i], new_node)

            if self.check_collision(curr_node, self.obstacle_list, self.robot_radius) == True:
                # if there is no collision ...
                cost = self.calc_new_cost(curr_node, new_node)
                costs.append(cost)
            else:
                # if there is a collision ...
                costs.append(float('inf')) # use cost=infinity
        ...
        # TODO Find which node has the minimum cost
        node_minC = self.node_list[near_inds[costs.index(min_cost)]]
        # TODO set that as the new parent of new_node using steer
        new_node = self.steer(node_minC, new_node)
        # TODO remember to update the cost of the updated new node
    ...
    def search_best_goal_node(self):
        ...
        for i in safe_goal_inds:
            safe_goal_costs.append(self.calc_new_cost(self.node_list[i], self.goal_node))
